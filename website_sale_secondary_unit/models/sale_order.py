...
class SaleOrder(models.Model):
    ...
    def _cart_find_product_line(self, product_id=None, line_id=None, **kwargs):
        ...
        """
        so_lines = super()._cart_find_product_line(product_id=product_id,
                                                   line_id=line_id,
                                                   **kwargs)
        if so_lines:
            if line_id:
        ...
        if not secondary_uom_id:
            ...
            if not product.allow_uom_sell:
                secondary_uom = (product.sale_secondary_uom_id
                                 or product.secondary_uom_ids[:1])
...
class SaleOrderLine(models.Model):
    ...
    def write(self, vals):
        ...
        Uom = self.env['uom.uom']
        for line in self:
            secondary_uom = ('secondary_uom_id' in vals
                             and SecondaryUom.browse(vals['secondary_uom_id'])
                             or line.secondary_uom_id)
            uom = ('product_uom' in vals and Uom.browse(vals['product_uom'])
                   or line.product_uom)
